    ...
    if leaf_cluster_idx is not None and leaf_cluster_idx.numel() > 0:
        ...
        for _, leaf_idx in enumerate(lerf_range):   # render each fine cluster
            ...
            if post_process and max_time > 0:
                ...
                #     K=int(filter_idx.sum()**0.5),
                # ).dists
                # def knn_with_torch(means3D, filter_idx, K):
                #     means3D_filtered = means3D[filter_idx]
                #     dists_matrix = torch.cdist(means3D_filtered, means3D_filtered)
                #     dists, _ = dists_matrix.topk(K, dim=-1, largest=False, sorted=False)
                #     return dists
                
                # def knn(means3D, filter_idx, K):
                #     # 获取过滤后的数据
                #     means3D_filtered = means3D[filter_idx].cpu().numpy().astype('float32')  # 转为 numpy 类型
                #     index = faiss.IndexFlatL2(means3D_filtered.shape[1])  # 使用 L2 距离的索引
                #     index.add(means3D_filtered)  # 添加数据点
                    
                #     # 获取距离和索引
                #     _, I = index.search(means3D_filtered, K)  # 搜索 K 近邻
                #     # 从索引获取距离
                #     dists, _ = index.search(means3D_filtered, K)
                    
                #     # 将距离转为 PyTorch tensor 并返回
                #     dists = torch.tensor(dists).to(means3D.device).float()  # 转为 float32 类型
                #     return dists
