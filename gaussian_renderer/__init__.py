...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, iteration,
            ...
            root_num=64, leaf_num=10, final_feats=None):  # k1, k2 
    """
    Render the scene. 
    ...
    if render_feat_map:
        ...
        ins_feat = (pc.get_ins_feat(final_feats, origin=origin_feat) + 1) / 2   # pseudo -> norm, else -> raw
    ...
    #             shs = _shs,  # feat
    #             colors_precomp = _colors_precomp1,  # feat
    ...
    #                 means2D = means2D[filter_idx],
    #                 shs = _shs,           # feat
    #                 colors_precomp = _colors_precomp2,  # feat
    #                 # shs = shs[filter_idx],  # rgb
    #                 # colors_precomp = None,  # rgb
    #                 opacities = opacity[filter_idx],
    #                 scales = scales[filter_idx] * rescale_factor,
    ...
    #     else:
    #         if isinstance(selected_leaf_id, list):
    #             lerf_range = selected_leaf_id
    #         else:
    ...
    #         # def expand_selection_using_convex_hull(means3D, filter_idx):
    #         #     if filter_idx.numel() == 0:
    #         #         return torch.zeros(means3D.shape[0], dtype=torch.bool, device=means3D.device)
    #         #     selected_points = means3D[filter_idx].cpu().numpy()
    #         #     hull = ConvexHull(selected_points)
    #         #     hull_vertices = selected_points[hull.vertices]
    #         #     delaunay = Delaunay(hull_vertices)
    #         #     simplex_indices = delaunay.find_simplex(means3D.cpu().numpy())
    #         #     mask = simplex_indices >= 0
    #         #     mask_tensor = torch.from_numpy(mask).to(means3D.device).reshape(-1,).bool()
    #         #     return mask_tensor
    #         # filter_idx = expand_selection_using_convex_hull(means3D.cpu(), filter_idx.cpu()).cuda()
    ...
    #         # NOTE object translation simple edition
    #         # translation = torch.tensor([-1.1,0.4,-1.1]).cuda()#第二维是上下，正为下;第一维是水平移动，正为朝外朝右；
    #         # pc._xyz[filter_idx]+=translation
    #         # filter_idx[:] = True
    #         # pc._features_dc[filter_idx] = torch.tensor([0.25, 0.88, 0.82]).view(1, 1, 3).cuda().expand_as(pc._features_dc[filter_idx])
    #         # pc.save_ply("./point_cloud.ply", [], filter_idx.cpu())
    ...
    if render_cluster and cluster_idx is not None:
        ins_feat = (pc.get_ins_feat(pc._ins_feat, origin=origin_feat) + 1) / 2   # pseudo -> norm, else -> raw
        # todo: rescale
        scale_filter = (scales < 0.1).all(dim=1)
        # scale_filter = (scales < 0.1).all(dim=1) & (opacity > 0.1).squeeze(-1)
        re_scale_factor = torch.ones_like(opacity)  # not used
        ...
        if isinstance(selected_root_id, int):
            selected_root_id=[selected_root_id]
        try:
            selected_root_id=selected_root_id.tolist()
        except AttributeError:
            selected_root_id=selected_root_id
        for _, idx in enumerate(selected_root_id):   # render each fine cluster
            # ignore the invisible clusters
            # if viewpoint_camera.bClusterOccur is not None and viewpoint_camera.bClusterOccur[selected_root_id] == False:
            #     continue
            # if idx != selected_root_id:#len(selected_root_id)==1 and idx != selected_root_id:
            #     continue

            # if selected_leaf_id is None:
            # if isinstance(selected_root_id, list):
            filter_idx = torch.isin(cluster_idx, torch.tensor(selected_root_id, device=cluster_idx.device))
                # filter_idx = cluster_idx == idx     # Render only the idx-th fine cluster
                # filter_idx = labels != value      # remove the idx-th fine cluster
            # else:
            #     filter_idx = cluster_idx == selected_root_id#(cluster_idx.unsqueeze(1) == selected_root_id).any(dim=1)
            ...
            if ins_feat.shape[-1] > 3:
                rendered_cluster2, _, _, _ = rasterizer(
                    means3D = means3D[filter_idx],
                    means2D = means2D[filter_idx],
                    shs = _shs,                          # rgb or feat
                    colors_precomp = _colors_precomp2,   # rgb or feat
            ...
            rendered_clusters.append(rendered_cluster)
            rendered_cluster_silhouettes.append(cluster_silhouette)
            if selected_root_id is not None and len(rendered_clusters) > 0:
                break
        if len(rendered_cluster_silhouettes) != 0:
            rendered_cluster_silhouettes = torch.vstack(rendered_cluster_silhouettes)
    else:
        rendered_clusters = None
        rendered_cluster_silhouettes =  None
        occured_id = None
    ...
    return {"render": rendered_image,
            ...
            "occured_id": occured_id,         # coarse cluster
