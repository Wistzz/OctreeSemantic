...
class GaussianModel:
    ...
    def get_ins_feat(self, final_feats, origin=False):
        # if len(self._ins_feat_q) == 0 or origin:
        ins_feat = final_feats
        # else:
        #     ins_feat = self._ins_feat_q
        ins_feat = torch.nn.functional.normalize(ins_feat, dim=1)
        return ins_feat
    ...
    def save_ply(self, path, save_q=[], filter_idx=None):
        ...
        vis_color = np.clip(vis_color, 0, 255).astype(np.uint8)
        ...
        if filter_idx!=None:
            elements = np.empty(xyz[filter_idx].shape[0], dtype=dtype_full)
            attributes = np.concatenate((xyz[filter_idx], normals[filter_idx], ins_feat[filter_idx],\
                                        f_dc[filter_idx], f_rest[filter_idx], opacities[filter_idx], scale[filter_idx], rotation[filter_idx],\
                                        r[filter_idx], g[filter_idx], b[filter_idx]), axis=1)
        else:
