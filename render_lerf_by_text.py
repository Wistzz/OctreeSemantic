...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, scene_name):
    ...
    kmeans, cluster_indices = load_code_book(os.path.join(model_path, "point_cloud", \
        f'iteration_{iteration}', "kmeans"))
    # root_cluster_indices = torch.from_numpy(root_cluster_indices).cuda()
    cluster_indices = torch.from_numpy(cluster_indices).cuda()
    ...
    root_num = cluster_indices.max() + 1
    leaf_num = root_num
    ...
    scene_texts = {
        ...
        "teatime": ['sheep', 'yellow pouf', 'stuffed bear', 'coffee mug', 'tea in a glass', 'apple', 
                ...
                'bag of cookies'],
        "IMG8304": [#"green fern in a white round pot",
                    "gray fabric armchair with a pillow",
                    # "gray fabric armchair with metal legs",
                    # "tropical green plant in a white square pot",
                    # "gray upholstered chair with a small pillow",
                    # "green fern in a white round pot",
                    # "white round side table",
                    # "pink vertical - striped wall"
                    ],
        "IMG8323": ['green giraffe', 'printer', 'chair', 'whiteboard', 'printer paper', 'vase']
    ...
    for t_i, text_feat in enumerate(query_text_feats):
        ...
        # kmeans = KMeans(n_clusters=8)  # 假设你希望聚成 8 个簇
        ...
        # leaf_lang_feat_cpu = leaf_lang_feat.cpu().numpy()
        # # 初始化 DBSCAN 模型
        # dbscan = DBSCAN(eps=0.2, min_samples=5)  # 你可以根据需要调整 eps 和 min_samples
        # labels = dbscan.fit_predict(leaf_lang_feat_cpu)
        # valid_indices = np.where(labels != -1)[0]

        # indices = np.where(labels == t_i)[0]
        # text_leaf_indices = torch.tensor(indices).cuda()
        ...
        for candidate_id in top_indices[0][1:]:
            ...
            max_feat = kmeans['ins_feat'][max_id]
            candi_feat = kmeans['ins_feat'][candidate_id]
        ...
        for idx, view in enumerate(tqdm(views, desc="Rendering progress")):         
            ...
            scene_gt_frames = {
                ...
                "IMG8304": ["00002", "00025", "00059", "00099", "00121", "00161"],
                "IMG8323": ["00002", "00025", "00059", "00099", "00121", "00161"]
            }
            candidate_frames = scene_gt_frames[scene_name]
            if  view.image_name not in candidate_frames:
                continue

            render_pkg = render(view, gaussians, pipeline, background, iteration, rescale=False, final_feats=gaussians._ins_feat)
            # RGB
            ...
            render_pkg = render(view, gaussians, pipeline, background, iteration,
                                ...
                                selected_root_id=text_leaf_indices.cuda(),#,torch.tensor([t_i]).cuda(),  # text query 所选择的 leaf id
            ...
            rendered_leaf_cluster_silhouettes = render_pkg["cluster_silhouettes"]
